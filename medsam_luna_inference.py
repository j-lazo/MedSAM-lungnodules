#!/usr/bin/env python3
"""
MedSAM slice-wise inference/evaluation for LUNA-style CT volumes.

This script is for bowang-lab/MedSAM, not MedSAM2.
MedSAM is image-only and box-prompt oriented, so this evaluator supports only
slice-wise prompted modes:

Modes
-----
1) image-single : one mask per prompt/mode
2) image-multi  : multimask output from the mask decoder; evaluates ch0/ch1/ch2
...
- the same perturbation is applied to point prompts and recentered boxes
...
Notes
-----
The official MedSAM inference path resizes images to 1024x1024, scales boxes to
that coordinate system, encodes the image once, and then runs the prompt encoder
and mask decoder directly. This script follows that pattern instead of using
SamPredictor.
"""
...
def build_experiment_name(args: argparse.Namespace) -> str:
    ...
    parts = [
        ...
        f"thr{format_float_for_name(args.mask_threshold)}",
...
def build_medsam_input_slice(image_array: np.ndarray, z: int, use_triplet_channels: bool = False) -> np.ndarray:
    """Return an HWC RGB uint8 image for MedSAM."""
...
def perturb_slice_prompt_dicts(slice_dicts: List[Dict], series_id: str, image_shape_hw: Tuple[int, int], rmax: int, seed: int) -> List[Dict]:
    ...
    for info in slice_dicts:
        ...
        for i in range(points.shape[0]):
            dx, dy = sample_prompt_shift_xy(seed, series_id, "medsam-image-slice", z, i, rmax)
...
# -----------------------------------------------------------------------------
# MedSAM model and inference
# -----------------------------------------------------------------------------

def build_medsam_model(args: argparse.Namespace, device: torch.device):
...
def resize_hwc_to_square(img_hwc: np.ndarray, image_size: int) -> np.ndarray:
    if sk_transform is not None:
        out = sk_transform.resize(
            img_hwc,
        ...
        out = cv2.resize(img_hwc, (image_size, image_size), interpolation=cv2.INTER_CUBIC).astype(np.uint8)
    return out


def medsam_preprocess_image(img_hwc: np.ndarray, image_size: int, device: torch.device) -> torch.Tensor:
    """Follow the official MedSAM_Inference.py preprocessing style."""
    img = resize_hwc_to_square(img_hwc, image_size=image_size)
    img = img.astype(np.float32)
    denom = np.clip(float(img.max() - img.min()), a_min=1e-8, a_max=None)
    img = (img - float(img.min())) / denom
    return torch.tensor(img).float().permute(2, 0, 1).unsqueeze(0).to(device)


def scale_points_to_medsam(points_xy: Optional[np.ndarray], H: int, W: int, image_size: int) -> Optional[np.ndarray]:
    if points_xy is None:
        return None
    scale = np.array([image_size / float(W), image_size / float(H)], dtype=np.float32)
    return np.asarray(points_xy, dtype=np.float32) * scale


def scale_box_to_medsam(box_xyxy: Optional[np.ndarray], H: int, W: int, image_size: int) -> Optional[np.ndarray]:
    if box_xyxy is None:
        return None
    scale = np.array([image_size / float(W), image_size / float(H), image_size / float(W), image_size / float(H)], dtype=np.float32)
    return np.asarray(box_xyxy, dtype=np.float32).reshape(1, 4) * scale


@torch.no_grad()
def medsam_predict_from_embedding(
    medsam_model,
    ...
    """
    Run MedSAM prompt encoder/mask decoder from a precomputed image embedding.

    Returns
    -------
    masks : np.ndarray
        Binary masks with shape [C, H, W]. C is 1 for single-mask mode and
        usually 3 for multimask mode.
    scores : np.ndarray
        Mask decoder IoU predictions with shape [C] if available.
    """
    if prompt_mode not in PROMPT_MODES:
        raise ValueError(f"Unknown prompt_mode: {prompt_mode}")

    use_points = prompt_mode in {"point", "point+box"}
    use_box = prompt_mode in {"box", "point+box"}

    points_tuple = None
    if use_points:
        if point_xy is None or point_labels is None:
            raise ValueError(f"{prompt_mode} requires point_xy and point_labels")
        point_1024 = scale_points_to_medsam(point_xy, H=H, W=W, image_size=image_size)
        labels = np.asarray(point_labels, dtype=np.int64)
        point_torch = torch.as_tensor(point_1024, dtype=torch.float, device=image_embedding.device)
        label_torch = torch.as_tensor(labels, dtype=torch.long, device=image_embedding.device)
        if point_torch.ndim == 2:
            point_torch = point_torch[None, :, :]  # [B=1, N, 2]
        if label_torch.ndim == 1:
            label_torch = label_torch[None, :]  # [B=1, N]
        points_tuple = (point_torch, label_torch)

    box_torch = None
    if use_box:
        if box_xyxy is None:
            raise ValueError(f"{prompt_mode} requires box_xyxy")
        box_1024 = scale_box_to_medsam(box_xyxy, H=H, W=W, image_size=image_size)
        box_torch = torch.as_tensor(box_1024, dtype=torch.float, device=image_embedding.device)
        # Official MedSAM_Inference.py expands boxes from [B,4] to [B,1,4].
    ...
    low_res_logits, iou_predictions = medsam_model.mask_decoder(
    ...
    probs = torch.sigmoid(low_res_logits)
    probs = F.interpolate(probs, size=(H, W), mode="bilinear", align_corners=False)
    probs_np = probs[0].detach().float().cpu().numpy()
    masks = (probs_np > float(mask_threshold)).astype(np.uint8)

    if iou_predictions is None:
        scores = np.full((masks.shape[0],), np.nan, dtype=np.float32)
    else:
        scores = iou_predictions[0].detach().float().cpu().numpy().astype(np.float32)
    return masks, scores
...
def predict_image_single(
        ...
        with torch.inference_mode(), safe_autocast(device, args.amp_dtype):
            ...
            for i in range(len(info["blobs"])):
                ...
                for mode in requested:
                    ...
                    slice_acc[mode] |= masks[0].astype(bool)
                    if sc.size > 0:
                        scores[mode].append(float(sc[0]))
    ...
    for mode in PROMPT_MODES:
        if scores[mode]:
            row[f"score_mean ({mode})"] = float(np.nanmean(scores[mode]))
...
def predict_image_multi(
    ...
    for key, vol in pred.items():
        ...
        if scores[key]:
            row[f"score_mean ({key})"] = float(np.nanmean(scores[key]))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="MedSAM slice-wise LUNA evaluation with deterministic prompt perturbations.")
    ...
    # Output paths
    parser.add_argument("--output-dir", default="results_medsam", help="Output root.")
    parser.add_argument("--run-name", default="luna_medsam", help="Short name embedded in the experiment folder.")

    # Model
    parser.add_argument("--model-type", default="vit_b", choices=["default", "vit_h", "vit_l", "vit_b"], help="MedSAM is usually vit_b.")
    parser.add_argument("--checkpoint", required=True, help="MedSAM checkpoint path, e.g. medsam_vit_b.pth")
    ...
    parser.add_argument("--image-size", type=int, default=1024, help="MedSAM inference resolution. Official MedSAM uses 1024.")
    parser.add_argument("--mask-threshold", type=float, default=0.5, help="Probability threshold after sigmoid.")

    # Mode selection
    parser.add_argument("--mode", choices=["image-single", "image-multi"], required=True)
    parser.add_argument(
        "--image-outputs",
        nargs="+",
        ...
        default=["box"],
        help=(
            "Prompt outputs to evaluate. MedSAM is box-prompt oriented, so the default is box. "
            "point and point+box are supported architecturally but may not match MedSAM's training setup."
        ),
    )
...
def main() -> None:
    ...
    medsam_model = build_medsam_model(args, device)
    ...
    for case_idx, (series_id, loaded, load_error) in enumerate(
        ...
        try:
            ...
            if args.mode == "image-single":
                row, pred_volumes = predict_image_single(series_id, image_array, gt_volume, medsam_model, args, device)
            elif args.mode == "image-multi":
                row, pred_volumes = predict_image_multi(series_id, image_array, gt_volume, medsam_model, args, device)
            else:
                raise ValueError(args.mode)
