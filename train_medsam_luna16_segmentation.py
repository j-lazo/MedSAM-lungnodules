#!/usr/bin/env python3
"""
Fully supervised positive-slice nodule segmentation with MedSAM encoder.
...
python train_medsam_luna16_segmentation_aug_decoder.py \
  ...
  --output-dir results/medsam_positive_slice_seg \
...
def build_medsam_input_slice(
    ...
    """Return H,W,3 uint8 input for MedSAM."""
...
class MedSAMEncoderSegmentationModel(nn.Module):
    ...
    def extract_features(self, x: torch.Tensor) -> torch.Tensor:
        x_sam = self.preprocess_for_medsam_encoder(x)
...
def load_checkpoint(model: MedSAMEncoderSegmentationModel, ckpt_path: Path, device: torch.device) -> Dict:
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train MedSAM encoder + supervised segmentation head on LUNA positive slices.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--run-name", default="luna_medsam_train")
